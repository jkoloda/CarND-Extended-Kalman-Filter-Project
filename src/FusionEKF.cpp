...
void FusionEKF::ProcessMeasurement(const MeasurementPackage &measurement_pack) {
    ...
    */
    ...
    if (measurement_pack.sensor_type_ == MeasurementPackage::RADAR) {
      ...
      */
      ...
      float slope = tan(phi);
      float speed = measurement_pack.raw_measurements_[2];
  ...
  if (measurement_pack.sensor_type_ == MeasurementPackage::RADAR) {
    // Radar updates
    // Set the transition (i.e. Jacobian in this case) and the error matrices
    ...
    // Laser updates
    // Set the transition and the error matrices
    ekf_.H_ = H_laser_;
    ekf_.R_ = R_laser_;
